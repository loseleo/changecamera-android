...
public class FloatWindowView extends FrameLayout implements IFloatView {
    ...
    private void initView() {
        ...
        floatView.findViewById(R.id.btn_done).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                tv_info.setText("justtry");
            }
        });
    ...
    private synchronized void updateWindowXYPosition(int x, int y) {
        Log.d("FloatWindowView","updateWindowXYPosition");
        if (mWindowManager != null) {
            mWindowParams.x = x;
            mWindowParams.y = y;
            mWindowManager.updateViewLayout(this, mWindowParams);
    ...
    private void showZoomView() {
        if (!isEdit) {//&& isSdkGt23 只有6.0及以上才显示缩放按钮
            ...
            videoViewWrap.setBackgroundColor(getResources().getColor(R.color.read_theme_yellow));
            isEdit = true;
        }
    }
    ...
    private void displayZoomView() {
        ...
        videoViewWrap.setBackgroundColor(getResources().getColor(R.color.dialog_red));
    }
